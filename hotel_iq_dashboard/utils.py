...
def _resolve_data_path(path: str) -> str:
    """Find the CSV whether the app is run from its own folder (local) or
    from a different working directory (e.g. Streamlit Cloud). Tries the
    given path as-is first, then falls back to a path relative to this
    script's own location.
    """
    if os.path.exists(path):
        return path
    fallback = os.path.join(_APP_DIR, path)
    if os.path.exists(fallback):
        return fallback
    raise FileNotFoundError(
        f"Could not find '{path}'. Checked '{os.path.abspath(path)}' and "
        f"'{fallback}'. Make sure the data/ folder was uploaded to the repo "
